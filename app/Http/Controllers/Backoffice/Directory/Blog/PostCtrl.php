...
class PostCtrl extends Controller
{
    ...
    {
        toastr();
        $data['url_data'] = route('admin:directory.post.data');
        $data['url_create'] = route('admin:directory.post.create');
//        $data['url_update'] = route('admin:directory.post.update');
        $data['url_delete'] = route('admin:directory.post.delete');
        $data['url_activated'] = route('admin:directory.post.active');
        $data['url_deactivated'] = route('admin:directory.post.nonactive');
        $data['title']='B2C Blog Post';
        return view('back-office.page.directory.blog.post.index',$data);
    }

    public function loadData()
    {
        $models = BlogPost::withGlobalScope('customer',new AudienceCustomerGeneralScope())->where('type_post', 'blog')->orderBy('created_at', 'desc');
...
//            })
            ->addColumn('action', function ($model) {
                $html = '<a href="'.route('admin:directory.post.edit',
    ...
    {
        $data['categorys'] = CategoryBlog::withGlobalScope('customer',new AudienceCustomerGeneralScope())->get();
        $data['tags'] = TagBlog::withGlobalScope('customer',new AudienceCustomerGeneralScope())->get();
        $data['url_save'] = route('admin:directory.post.save');
        $data['title'] = 'New B2C Blog Post';
...
//                }
            ...
            \DB::commit();
            return apiResponse(200, 'Success add Blog Post', ['redirect' => route('admin:directory.post.index')]);
    ...
    {
        $data['post'] = BlogPost::withGlobalScope('customer',new AudienceCustomerGeneralScope())->whereId($id)->first();
        $data['categorys'] = CategoryBlog::withGlobalScope('customer',new AudienceCustomerGeneralScope())->get();
        $data['tags'] = TagBlog::withGlobalScope('customer',new AudienceCustomerGeneralScope())->get();
        $data['title'] = 'Edit B2C Blog Post';
    ...
    {
        if (!$voucher = BlogPost::withGlobalScope('customer',new AudienceCustomerGeneralScope())->whereId($request->id)->first()) {
    ...
    {
        $del = BlogPost::withGlobalScope('customer',new AudienceCustomerGeneralScope())->whereId($request->id)->first();
        if (!$del){
