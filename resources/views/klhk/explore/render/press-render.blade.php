@forelse($press as $blog)
    <div class="post">
        <div class="post-content-wrapper">
            <figure class="image-container">
                @if ($blog->image_blog)
                    <a href="{{route('pres_releases.detail',['slug'=>$blog->slug])}}"><img
                                src="{{ asset($blog->image_blog) }}" alt="{{ $blog->alternative }}"/></a>
                @else
                    <a href="{{route('pres_releases.detail',['slug'=>$blog->slug])}}"><img
                                src="{{ asset('img/no-banner-image.png') }}" alt="{{ $blog->alternative }}"
            ...
            <div class="details">
                ...
                <h2 class="entry-title">
                    @if (App::getLocale() ==='id')
                        <a href="{{route('pres_releases.detail',['slug'=>$blog->slug])}}">{{ $blog->title_ind }}</a>
                    @else
                        <a href="{{route('pres_releases.detail',['slug'=>$blog->slug])}}">{{ $blog->title_eng }}</a>
                    @endif
                </h2>
...
@endforelse
<div class="pagination-blog hide" data-total="{{$press->total()}}" data-current="{{$press->currentPage()}}"
     data-nextPage="{{$press->nextPageUrl()?$press->currentPage()+1:null}}">
</div>