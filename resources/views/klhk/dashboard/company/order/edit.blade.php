...
@section('additionalStyle')
    ...
    <link href="{{ asset('materialize/js/plugins/bootstrap-touchspin/css/jquery.bootstrap-touchspin.min.css') }}"
          rel="stylesheet"/>
    <link href="{{ asset('klhk.dest-operator/css/order_edit.css') }}" rel="stylesheet">
    <style>
        label.error {
            position: absolute
        }
                            ...
                            <div class="order-profile-header">
                                <div class="order-user">
                                    ...
                                    <div class="order-user-info">
                                        <h3 class="mb-half">{{$order->customer_info->first_name}} @if($order->customer_info->last_name){{$order->customer_info->last_name}}@endif</h3>
                                ...
                                <div class="row">
                                    <div class="col-sm-12 col-md-12 col-lg-4 col-xl-3">
                                        ...
                                        @if ($order->id_voucher)
                                            ...
                                            <h4 class="font-weight-bold">{{$order->order_detail->currency}} {{ number_format($order->voucher_amount,0) }}</h4>
                                        @endif

                                        @if ($order->product_discount > 0)
                                            <h6>{{ trans('order_provider.product_discount') }}</h6>
                                            <h4 class="font-weight-bold">{{$order->order_detail->currency}} {{ number_format($order->product_discount,0) }}</h4>
                                        @endif
                                        @if($order->fee > 0)
                                        ...
                                        <h3>{{$order->order_detail->currency}} {{ number_format($order->total_amount,0) }}</h3>
                                    </div>
                                    <div class="col-sm-12 col-md-12 col-lg-8 col-xl-9">
                                        <div class="order-contact">
                                            ...
                                            @endif
                                            <h5>
                                                <span class="fa fa-calendar"></span>{{ date('d M Y',strtotime($order->order_detail->schedule_date)) }}
                                            </h5>
                                            <h5>
                                                <span class="fa fa-clock-o"></span>{{ $order->order_detail->product->schedule[0]->start_time }}
                ...
                {{-- <div class="widget card" id="order-guest">
                    <div class="widget-header widget-collapse" id="header-order-guest" data-toggle="collapse"
                        ...
                        <i class="float-right fa fa-chevron-down rotate text-white"
                            id="header-order-guest-chevron"></i>
                    </div>
                            ...
                            <div class="row mt-3">
                                @if($order->customer_info->identity_number)
                                    <div class="col-md-4">
                                        ...
                                        <input type="text" class="form-control" name="phone"
                                                id="phone"
                                                value="{{ $order->customer_info->identity_number_type }}"
                                                readonly/>
                                    </div>
                                    <div class="col-md-4">
                                        ...
                                        <input type="text" class="form-control" name="phone"
                                                id="phone"
                                                value="{{ $order->customer_info->identity_number }}"
                                                readonly/>
                                    </div>
                                @endif
                                @if($order->customer_info->emergency_number )
                                    <div class="col-md-4">
                                        ...
                                        <input type="text" class="form-control" name="phone"
                                                id="phone"
                                                value="{{ $order->customer_info->emergency_number }}"
                                                readonly/>
                                    </div>
                                @endif
                            </div>
                                    ...
                                    <div class="col-md-6">
                                        @if($order->customer_info->address)
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control resize-none" name="address"
                                                            id="address"
                                                            rows="8"
                                            ...
                                            @if($order->customer_info->city)
                                                ...
                                                <div class="col-12">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control"
                                                                value="{{ $order->customer_info->city->state->state_name }}"
                                                                readonly/>
                                                    </div>
                                                </div>
                                                <div class="col-12">
                    ...
                    <div class="widget card" id="custom_info">
                        ...
                        <div class="widget-content collapse pt-0" id="content-custom-info">
                                    ...
                                    <div class="col-sm-12 col-md-12 col-lg-12">
                                        @if ($order->order_detail->customDetail->unique('participant')->count() > 1)
                                            <ul class="nav nav-pills">
                                                @foreach ($order->order_detail->customDetail->unique('participant') as $p)
                                                    <li class="nav-item">
                                                        <a class="nav-link {{ $p->participant == 0 ? ' active' : '' }}" id="add{{ $p->participant }}-tab" data-toggle="tab" href="#add{{ $p->participant }}" role="tab" aria-controls="add{{ $p->participant }}" aria-selected="{{ $p->participant == 0 ? 'true' : 'false' }}">
                                        ...
                                        <div class="tab-content pt-2" id="nav-tabContent">
                                            @foreach ($order->order_detail->customDetail->groupBy('participant') as $key => $value)
                                                <div class="tab-pane fade{{ $key == 0 ? ' show active' : '' }}" id="add{{ $key }}" role="tabpanel" aria-labelledby="add{{ $key }}-tab">
                                                    <div class="row">
                                                            ...
                                                            @switch($input->type_custom)
                                                                ...
                                                                @case('document')
                                                                    <div class="form-group">
                                                                        ...
                                                                        <div class="input-group">
                                                                            <input type="text" class="form-control border-left" disabled value="{{ ucfirst($input->type_custom) }}">
                                                                            <div class="input-group-append">
                                                                            <span class="input-group-text border-right">
                                                                                <a href="{{ $input->value }}">{{ trans('product_provider.preview') }}</a>
                                                                            </span>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                    @break
                                                                @case('checkbox')
                                                                    <div class="mt-2"><strong class="text-muted">{{ $input->label_name }}</strong></div>
                                                                ...
                                                                @case('country')
                                                                    <div class="form-group">
                                                                        <label>{{ $input->label_name }}</label>
                                                                        <input type="text" class="form-control" value="{{ $location[$input->type_custom][(int) $input->value] }}" disabled>
                                                                    </div>
                                                                    @break
                                                                @default
                                                                    <div class="form-group">
                                                                        <label>{{ $input->label_name }}</label>
                                                                        <input type="text" class="form-control" value="{{ $input->value }}" disabled>
                                                                    </div>
                                                                    @break
                                                            @endswitch
                                                            </div>
                            ...
                            <div class="text-center mt-3">
                                <a class="btn bg-success text-white border-0" href="{{ route('company.order.export_custom_detail', ['invoice_no' => $order->invoice_no]) }}">
                                ...
                                @if ($order->order_detail->customDetail->filter(function ($value) {
                                    ...
                                    <a class="btn bg-success text-white" href="{{ route('company.order.download_custom_detail', ['invoice_no' => $order->invoice_no]) }}">
                    ...
                    <div class="widget card" id="order-guide">
                        ...
                        <div class="widget-content">
                            <div class="form-group-no-margin">
                                <div class="row mb-3">
                                    ...
                                    <div class="col-md col-sm-12 mt-lg-0 mt-4">
                                        ...
                                        {!! Form::text('guide_phone_number',null,['class'=>'form-control number', 'maxlength'=>'15']) !!}
                                    </div>
                                    <div class="col-md-auto md-and-center mt-lg-auto mt-3">
                                        <button type="button" class="btn btn-block bg-green-klhk"
                                ...
                                @foreach($order->guides()->orderBy('created_at')->get() as $guide)
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-md col-sm-12 mt-lg-0 mt-4">
                                            {!! Form::label('guide_phone_number[]',trans('order_provider.guide_phone')) !!}
                                            {!! Form::text('guide_phone_number[]',$guide->guide_phone_number,['class'=>'form-control number','readonly','maxlength'=>'15']) !!}
                                        </div>
                                        <div class="col-md-auto md-and-center mt-lg-auto mt-3">
                            ...
                            @if($order->guides()->orderBy('created_at')->count()>0)
                                <div class="row">
                                    <div class="col">
                                        <button type="button" class="btn col-lg-auto bg-green-klhk float-right mt-3"
                                                id="btn-send" data-resend="{{ trans('order_provider.resend_to_customer') }}" data-send="{{ trans('order_provider.send_to_customer') }}">
                                        </button>
                                    </div>
                                </div>
                            @else
                                <div class="row">
                                    <div class="col">
                                        <button type="button"
                                                class="btn col-lg-auto bg-green-klhk float-right mt-3" id="btn-send" data-resend="{{ trans('order_provider.resend_to_customer') }}" data-send="{{ trans('order_provider.send_to_customer') }}">
                                        </button>
                                    </div>
                                </div>
                            @endif
...
@endif -->
        ...
        @if($order->payment->payment_gateway=='Cash On Delivery')
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    {!! Form::label('','&nbsp;') !!}
                    <button class="btn bg-green-klhk btn-block">{{trans('order_provider.submit')}}</button>
                </div>
            </div>
...
{{--    <script src="{{ asset('js/sweetalert2.all.min.js') }}"></script>--}}
    ...
    function sendButton(e){
        ...
        if(e == 1){
            ...
            send_button.removeClass('btn-warning').addClass('bg-green-klhk');
        } else {
            send_button.text(send_button.attr('data-resend'));
            send_button.removeClass('bg-green-klhk').addClass('btn-warning');
            // #ffc107
        }
    }
