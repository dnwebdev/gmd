            ...
            <div class="row">
                            ...
                            @case('PENDING')
                                <svg id="wrap" width="300" height="300">
                                  ...
                                  <svg>
                                    ...
                                    <circle cx="150" cy="150" r="115" style="fill:#4cb050"/>
                                    <path style="stroke: #4cb050; stroke-dasharray:820; stroke-dashoffset:820; stroke-width:18; fill:transparent" d="M150,150 m0,-130 a 130,130 0 0,1 0,260 a 130,130 0 0,1 0,-260">
                                  ...
                                  <svg>
                                    <!-- upper part -->
                                    <polygon id="upper" points="120,125 180,125 150,147" style="fill:#4cb050;">
                                    ...
                                    <!-- falling sand -->
                                    <path id="line" stroke-linecap="round" stroke-dasharray="1,4" stroke-dashoffset="200.00" stroke="#4cb050" stroke-width="2" d="M150,150 L150,198">
                                      <!-- running sand -->
                                      ...
                                      <!-- last drop -->
                                      <animate attributeName="stroke" dur="3s" keyTimes="0; 0.65; 0.8; 1" values="#4cb050;#4cb050;transparent;transparent" to="transparent" repeatCount="indefinite"/>
                                    </path>
                                    
                                    <!-- lower part -->
                                    <g id="lower">
                                      <path d="M150,180 L180,190 A28,10 0 1,1 120,190 L150,180 Z" style="stroke: transparent; stroke-width:5; fill:#4cb050;">
                ...
                <div class="col-12 mt-4">
                    <div class="bg-white p-4">
                        ...
                        <div class="text-muted mt-3">Waktu Pemesanan: {{ $order->created_at }}</div>
                        <hr />
                        <div class="my-3">
                            <div class="text-muted">@lang('customer.kredivo.form.first_name')</div>
                            <h5>{{ $order->payment->kredivo->first_name }}</h5>
                        </div>
                        <div class="my-3">
                            <div class="text-muted">@lang('customer.kredivo.form.last_name')</div>
                            <h5>{{ $order->payment->kredivo->last_name }}</h5>
                        </div>
                        <div class="my-3">
                            <div class="text-muted">@lang('customer.kredivo.form.phone_number')</div>
                            <h5>{{ $order->payment->kredivo->phone_number }}</h5>
                        </div>
                        <div class="my-3">
                            <div class="text-muted">@lang('customer.kredivo.form.email')</div>
                            <h5>{{ $order->payment->kredivo->email }}</h5>
                        </div>
                        <div class="my-3">
                            <div class="text-muted">@lang('customer.kredivo.form.address')</div>
                            <h5>
                                {{ $order->payment->kredivo->address }}, {{ $order->payment->kredivo->city->city_name }}, {{ $order->payment->kredivo->city->state->state_name }} {{ $order->payment->kredivo->postal_code }}
                            </h5>
                        </div>
                        <div class="my-3">
                            <div class="text-muted">Durasi Cicilan</div>
                            <h5>{{ preg_replace_callback('/\{\{(.*)\}\}/', function($matches) {
                                return trans('duration.'.$matches[1]);
                            }, \App\Models\PaymentKredivo::$durations[$order->payment->kredivo->installment_duration]) }}</h5>
                        </div>
                    </div>
                </div>
            </div>
