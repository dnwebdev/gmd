...
<table align="center" width="680px" cellpadding="5" cellspacing="0">
    ...
    <tr>
        <td colspan="3" class="b-primary">
            <table align="center" width="680px" cellpadding="5" cellspacing="0">
                ...
                <tr>
                    <td colspan="3">
                        <p>{{ $order->order_detail->product_description }}</p>
                    </td>
                </tr>
                <tr>
                    <td colspan="3">
                        <b>Start Date</b> : {{ date('M d, Y',strtotime($order->order_detail->schedule_date)) }}
                    </td>
                </tr>
                <tr>
                    <td colspan="3">
                        <b>Duration</b>
                        : {{ $order->order_detail->duration }} {{ $order->order_detail->duration_type_text}}
                    </td>
                </tr>
                @if($order->order_detail->product && $order->order_detail->product->schedule->count()>1)
                    <tr>
                        <td colspan="3">
                            <b>Operational Hours</b> : {{ $order->order_detail->product->schedule[0]->start_time }}
                ...
                @if($order->order_detail->city)
                    <tr>
                        <td colspan="3">
                            <b>Location</b> : {{ $order->order_detail->city->city_name }}
                        </td>
                    </tr>
                @endif
                <tr>
    ...
    <tr>
        <td colspan="3" class="b-secondary">
            <table align="center" width="680px" cellpadding="5" cellspacing="0">
                <tr>
                    <td align="left" style="border: 1px solid #ddd"><b>Guest</b></td>
                    <td align="center" style="border: 1px solid #ddd"><b>Price</b></td>
                    <td align="center" style="border: 1px solid #ddd"><b>Total</b></td>
                </tr>

                @if($order->order_detail->adult > 0)
                    <tr>
                        <td align="left"
                            style="border: 1px solid #ddd">{{ $order->order_detail->adult }} {{optional($order->order_detail->unit)->name}}</td>
                        <td align="right"
                            style="border: 1px solid #ddd">{{ number_format($order->order_detail->adult_price,0) }}</td>
                        <td align="right"
                            style="border: 1px solid #ddd">{{ number_format($order->order_detail->adult_price * $order->order_detail->adult,0) }}</td>
                    </tr>
                @endif
                ...
                @if($order->order_detail->discount_amount > 0 || $order->voucher_amount > 0)
                    ...
                    @if($order->order_detail->discount_amount > 0)
                        <tr>
                            ...
                            <td align="right"
                                style="border: 1px solid #ddd">{{ number_format($order->order_detail->discount_amount,0) }}</td>
                        </tr>
                    @endif
