...
@section('subheader')
    <div class="d-flex align-items-center">
        <div class="mr-auto">
            <h3 class="m-subheader__title ">Need Approval KYC</h3>
        </div>
    ...
    <script>
            ...
            if ($('tr#child-'+t.data('id')).length===0){
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        tb+='<tfoot>';
                        tb += '<tr>';
                        tb += '<td colspan="4" class="text-center">';
                        tb+='<button class="btn btn-danger btn-reject mr-2" data-id="'+t.data('id')+'">Reject</button>'
                        tb+='<button class="btn btn-success btn-approve" data-id="'+t.data('id')+'">Approve</button>'
                        tb += '</td>';
                        tb += '</tr>';
                        tb+='</tfoot>';
        ...
        $(document).on('click','.btn-approve', function () {
            ...
            $.ajax({
                ...
                error:function (e) {
                    toastr.error(e.reponseJSON.message,'{{__('general.whoops')}}')
                    dt.api().ajax.reload(null,false)
                }
            })
        })
