...
@section('subheader')
    <div class="d-flex align-items-center">
        <div class="mr-auto">
            <h3 class="m-subheader__title ">Press Releases</h3>
        </div>
...
@section('content')
        ...
        <div class="m-portlet__body  ">
            <div class="row ">
                <div class="col-12 mb-3 text-right">
                    <a id="btn-add" href="{{route('admin:directory.press-releases.create')}}"
                       class="btn btn-sm btn-brand"><i class="fa fa-plus"></i> Add Press Releases</a>
                </div>
                <div class="col-12">
    ...
    <div class="modal fade" id="modal-delete" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            {!! Form::open(['url'=>route('admin:directory.press-releases.delete'),'style'=>'width:100%','files'=>true]) !!}
...
@section('scripts')
    ...
    <script>
        let dt = $('#dt').dataTable({
            ...
            "responsive": true,
            "ajax": {
                url: "{{route('admin:directory.press-releases.data')}}",
                type: "GET",
            },
            "columns": [
                {
        ...
        $(document).on('click', '.btn-active', function () {
            let btn = $(this);
            $.ajax({
                url: '{{route('admin:directory.post.active')}}',
                type: 'POST',
                dataType: 'json',
                data: {id: btn.data('id')},
        ...
        $(document).on('click', '.btn-nonactive', function () {
            let btn = $(this);
            $.ajax({
                url: '{{route('admin:directory.post.nonactive')}}',
                type: 'POST',
                dataType: 'json',
                data: {id: btn.data('id')},
                success: function (data) {
