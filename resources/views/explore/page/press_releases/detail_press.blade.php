...
@section('content')
    <section id="content">
        <div class="container">
            <div class="row">
                @include('explore.page.press_releases.sidebar-pres')
                            ...
                            <div class="detail blog-infinite">
                                <div class="post">
                                    <figure class="image-container">
                                        @if ($blog->image_blog)
                                            <a href="{{route('pres_releases.detail',['slug'=>$blog->slug])}}"><img
                                    ...
                                    <div class="single-navigation block">
                                        <div class="row">
                                            @if ($previous)
                                            <div class="col-xs-6"><a href="{{ route('pres_releases.detail', $previous->slug) }}" rel="prev"
                                                                     class="button btn-large sky-blue1 prev full-width"><i
                                                            class="soap-icon-longarrow-left"></i><span> {{trans('explore-lang.press_release.previous')}}</span></a>
                                            </div>
                                            @endif
                                            @if ($next)
                                            <div class="col-xs-6"><a rel="next" href="{{ route('pres_releases.detail', $next->slug) }}"
                                                                     class="button btn-large sky-blue1 next full-width"><span>{{trans('explore-lang.press_release.next')}}</span><i
                                    ...
                                    <div class="about-author block">
                                        <h2>{{trans('explore-lang.press_release.about_author')}}</h2>
    ...
    <script>
        ...
        function render() {
            ...
            if (page !== undefined || page !== '' || page !== null) {
                tjq.ajax({
                    url: "{{route('pres_releases.render')}}",
                    type: "POST",
                    data: data,
                    dataType: 'html',
        ...
        tjq(document).on('submit', '#search-press', function (e) {
            e.preventDefault();
            page = 1;
